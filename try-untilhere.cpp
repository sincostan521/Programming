...
void alienMove(int rows, int columns, string blocks[5][19], string command)
{
    ...
    int Xalien;
    int Yalien;
    for(Xalien = 0; Xalien < rows + 1; Xalien++)
    {
        for(Yalien = 0; Yalien < columns + 1; Yalien++)
        {
            if(blocks[Xalien][Yalien] == "A");
        }
    }
...
int main()
{
    ...
    while (true)
    {
        ...
        while (command != cmup && command != cmdown && command != cmleft && command != cmright)
        {
            cout << "Please type in properly! => ";
            cin >> command;
        }

        if (command == cmup || command == cmdown || command == cmleft || command == cmright)
        {
            cout << "hits a border"
                 << " ...";
        }

        alienMove(dsg2, dsg1, blocks, command);
    }
